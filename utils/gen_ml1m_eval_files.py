from load_ml1m_data import load_movie, load_user, load_interactions
from ml1m_data import interact_split, process_items
from submit import *
...
def gen_label_data(debug = 0):
  ...
  format_submit(seq_tr, 'ml1m_historical_train.csv')  
  format_submit(seq_va, 'ml1m_res_T.csv')
  format_submit(seq_te, 'ml1m_res_T_test.csv')

  seq_va_tr = seq_va
  for u in seq_tr:
    if u in seq_va:
      seq_va_tr[u] = seq_va[u] +','+ seq_tr[u]
  format_submit(seq_va_tr, 'ml1m_historical_train_test.csv')
