...
cd /home/nlg-05/xingshi/lstm/tensorflow/recsys/word2vec/
...
def main(acct = 0):
    ...
    # after 40, ni > 1
    # funcs = [dataset, data_dir, batch_size, size, dropout, learning_rate, loss, ckpt, item_vocab_size, ta, n_i, n_skip, win, use_out_items, after40]
    # template = ["xing", "$data_full", 64, 32, 0.5, 3, 'ce', 4000, 50000, 0, 3, 0, 5, True, True]
    ...
    # template_ml = ["ml", "$data_ml", 64, 32, 0.5, 1, 'ce', 4000, 13000, 0, 1, 0, 5, True, 12, True]

    # ml1m
    # funcs = [dataset, data_dir, batch_size, size, dropout, learning_rate, loss, ckpt, item_vocab_size, ta, n_i, n_skip, win, use_out_items, epoch, test]
    # template_ml = ["ml1m", "$data_ml1m", 64, 32, 0.5, 1, 'ce', 4000, 3100, 0, 1, 0, 5, True, 27, True]    

    # yelp
    funcs = [dataset, data_dir, batch_size, size, dropout, learning_rate, loss, ckpt, item_vocab_size, ta, n_i, n_skip, win, use_out_items, wFeat, epoch, test]
    template = ["yelp", "$data_yelp", 64, 32, 0.5, 1, 'ce', 4000, 80000, 0, 1, 0, 5, False, False, 24, True]
    ...
    _ot = [False]
    # after 40, woF
    # funcs = [dataset, data_dir, batch_size, size, dropout, learning_rate, loss, ckpt, item_vocab_size, ta, n_i, n_skip, win, use_out_items, after40, wFeat, epoch, test]
    # template = ["xing", "$data_full", 64, 32, 0.5, 3, 'ce', 4000, 50000, 0, 1, 0, 5, True, True, False, 1000, False]
    ...
    if acct == 0: # isicvl
        ...
        head = head_hpc1 # xing hpc
    elif acct == 3:
        head = head_hpc3 # k -hpc
    else:
        head = head_hpc2 # xing hpc i80

    # train
    for para in paras:
        name, cmd = get_name_cmd(para)
        if acct == 0:
            cmd = "/nfs/isicvlnas01/share/anaconda/bin/python run_cbow.py " + cmd
        else:
            cmd = "python run_cbow.py " + cmd
    ...
    # batch_job_name = 'cbow_xing_recommend_h32_oT_ni'
    # batch_job_name = 'cbow_ml_recommend_h32_oT_ni'
    # batch_job_name = 'cbow_ml1m_recommend_h32_oT_ni'
    # batch_job_name = 'cbow_ml1m_recommend_h_oT_ni'
    # batch_job_name = 'cbow_yelp_recommend_h16_oT_ni_lr'
    # batch_job_name = 'cbow_yelp_recommend_h32_oT_ni_lr'
    # batch_job_name = 'cbow_yelp_recommend_h32_oF_ni_lr'
    # batch_job_name = 'ml1m_cbow_recommend_oTni3_test1'
    # batch_job_name = 'cbow_ml_recommend_h32_oT_ni'
    # batch_job_name = 'cbow_yelp_recommend_h32_wof'
    # batch_job_name = 'cbow_xing_recommend_h32_wof'
    # batch_job_name = 'cbow_xing_recommend_test_wof'
    batch_job_name = 'cbow_yelp_recommend_wof_test1'

    cmds = ''
    for para in paras:
        name, cmd = get_name_cmd(para)
        if acct == 0:
            cmd = "/nfs/isicvlnas01/share/anaconda/bin/python run_cbow.py " + cmd + ' --recommend True'
        else:
            cmd = "python run_cbow.py " + cmd + ' --recommend True'    
        cmds += cmd + '\n'
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and int(sys.argv[1]) == 1: # hpc xing
        main(1)
    elif len(sys.argv) > 1 and int(sys.argv[1]) == 2: # i80
        main(2)
    elif len(sys.argv) > 1 and int(sys.argv[1]) == 3: # k hpc
        main(3)
    else:
        main()    
