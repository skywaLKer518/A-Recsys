...
class LatentProductModel(object):
  def __init__(self, user_size, item_size, size,
    ...
    print("construct output embedding layers")
    # full vocabulary item embedding
    (indices_cat, indices_mulhot, embs_cat, embs_mulhot, biases_cat, 
      biases_mulhot, projs_cat, projs_mulhot) = self.full_output_layer(
      item_attributes, user_model_size)

    print("construct postive/negative items/scores ")
    pos_embs_item_cat, pos_embs_item_mulhot = self.EmbeddingLayer3(
      self.item_input, batch_size, size, item_attributes)
    neg_embs_item_cat, neg_embs_item_mulhot = self.EmbeddingLayer3(
      self.neg_item_input, batch_size, size, item_attributes)
    ...
    for i in xrange(item_attributes.num_features_cat):
      ...
      innerp = tf.matmul(proj_user_drop, embs_cat[i]) + biases_cat[i] # mb by V_f
    ...
    for i in xrange(item_attributes.num_features_mulhot):
      proj_user = tf.matmul(embedded_user, projs_mulhot[i])
      proj_user_drop = tf.nn.dropout(proj_user, self.keep_prob)
      innerp = tf.matmul(proj_user_drop, embs_mulhot[i]) + biases_mulhot[i]
  ...
  def mapped(self, attributes, prefix):
  ...
  def full_output_layer(self, attributes, hidden_size, scope = None):
    indices_cat, indices_mulhot = [], []
    embs_cat, embs_mulhot, biases_cat, biases_mulhot = [], [], [], []
    ...
    with vs.variable_scope(scope or type(self).__name__):
      with ops.device("/cpu:0"):
        ...
        attributes, prefix=prefix) # NOT trainable
        feats_cat_tr, _, _ , _ = self.mapped_item_tr(attributes, prefix=prefix)

        embs_cat, embs_mulhot = self.embedded(attributes, 
          prefix=prefix, transpose=True)


        # create indices
        # N = self.item_size  
        # U = tf.constant(np.array(range(N)))
        # for i in range(attributes.num_features_cat):
        #   idx = array_ops.reshape(embedding_ops.embedding_lookup(feats_cat[i], 
        #     U), [-1])
        #   indices_cat.append(idx)
        indices_cat = feats_cat_tr

        # print("num of features_cat : %d" % attributes.num_features_cat)
    ...
    return (indices_cat, indices_mulhot, embs_cat, embs_mulhot, biases_cat, 
      biases_mulhot, projs_cat, projs_mulhot)
  ...
  def self.EmbeddingLayer3(self.item_input, batch_size, size, item_attributes):
    '''
    similar as EmbeddingLayer2, no concatenation
    '''
    embs_item_cat, embs_item_mulhot = [], []
  ...
  def EmbeddingLayer2(self, input_user, mb, size, attributes, scope = None):
    with vs.variable_scope(scope or type(self).__name__):
      with ops.device("/cpu:0"):
        ...
        prefix = 'user'
        feats_cat, feats_mulhot, mulhot_starts, mulhot_lengths = self.mapped(
          attributes, prefix=prefix) # all NOT trainable
        embs_cat, embs_mulhot = self.embedded(attributes, prefix=prefix)
