...
class LatentProductModel(object):
  def __init__(self, user_size, item_size, size,
    ...
    m = embed_attribute_device.EmbeddingAttribute(user_attributes, item_attributes, mb, 
      self.n_sampled, 0, False, item_ind2logit_ind, logit_ind2item_ind, devices=self.devices)
    self.att_emb = m
    embedded_user, user_b = m.get_batch_user(self.keep_prob, False, device=self.devices[0])
    
    pos_embs_item, pos_item_b = m.get_batch_item('pos', batch_size, device=self.devices[0])
    pos_embs_item = tf.reduce_mean(pos_embs_item, 0)

    neg_embs_item, neg_item_b = m.get_batch_item('neg', batch_size, device=self.devices[0])
    ...
    if self.n_sampled is not None:
      sampled_logits = m.get_prediction(embedded_user, 'sampled', device=self.devices[-1])
    # if self.n_sampled is not None:
    ...
    logits = m.get_prediction(embedded_user, device=self.devices[-1])

    loss = self.loss_function
    if loss in ['warp', 'ce']:
      batch_loss = m.compute_loss(logits, self.item_target, loss, device=self.devices[-1])
    elif loss in ['mw']:
      batch_loss = m.compute_loss(sampled_logits, self.pos_score, loss, device=self.devices[-1])
      batch_loss_eval = m.compute_loss(logits, self.item_target, 'warp', device=self.devices[-1])

    elif loss in ['bpr', 'bpr-hinge']:
      batch_loss = m.compute_loss(neg_pos, self.item_target, loss, device=self.devices[-1])
    else:
      print("not implemented!")
      exit(-1)
    if loss in ['warp', 'mw']:
      self.set_mask, self.reset_mask = m.get_warp_mask(device=self.devices[-1])
