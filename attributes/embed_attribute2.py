...
class EmbeddingAttribute(object):
  def __init__(self, user_attributes, item_attributes, n_sampled, 
  ...
  def get_target_score(self, latent, inds, mb, device='/gpu:0'):
    ...
    cat_l, mulhot_l, i_bias = self._get_embedded(item_emb_cat, item_embs_mulhot, 
      i_biases_cat, item_biases_mulhot, inds, mb, 
    ...
    with tf.device(device):
      if concat:
        embedded_user, user_b = self._get_embedded(self.user_embs_cat, 
          ...
          mb=batch_size, attributes=self.user_attributes, prefix='user', 
        ...
        user_cat, user_mulhot, user_b = self._get_embedded(
          ...
          inds=u_inds, mb=batch_size, attributes=self.user_attributes, 
  ...
  def compute_loss(self, logits, item_target, batch_size=0, loss='ce', device='/gpu:0'):
    assert(loss in ['ce', 'mce', 'warp', 'mw', 'bbpr', 'bpr', 'bpr-hinge'])
    with tf.device(device):
      if loss == 'ce':
        return tf.nn.sparse_softmax_cross_entropy_with_logits(logits, item_target)
      elif loss == 'warp':
        return self._compute_warp_loss(logits, item_target, batch_size)
      elif loss == 'mw':
        return self._compute_mw_loss(logits, item_target, batch_size)  
      elif loss =='bbpr':
        return self._compute_bbpr_loss(logits, item_target, batch_size)
      elif loss == 'bpr':
  ...
  def _compute_bbpr_loss(self, logits, item_target, mb):
    loss = 'bbpr'
  ...
  def _compute_warp_loss(self, logits, item_target, mb):
    loss = 'warp'
    if loss not in self.mask:
      self._prepare_warp_vars(loss)
    V = self.logit_size
  ...
  def _compute_mw_loss(self, logits, item_target, mb):
